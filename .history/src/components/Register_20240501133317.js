...
const Register = () => {
    ...
    const schema = Joi.object({
        ...
        password: Joi.string().pattern(passwordRegex).required().messages({
            ...
            "any.only": "Password must contain at least one lowercase letter, one uppercase letter, and one digit"
        }),
    ...
    const onSubmit = (data) => {
        // Remove country from the data object if it's empty
        if (data.country === "") {
            delete data.country;
        }
        ...
        if (!data.country) {
            setFormErrors({ ...formErrors, country: "Country selection is required" });
            return;
        }
